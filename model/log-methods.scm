...
(model-method (<log-introspection>) (my-list self)
				  (let ((mil (my 'introspection-list)))
					 (if (procedure? mil)
						  (filter mil (agent-kcall self 'runqueue))
						  mil)
				  ))
...
(model-method (<log-introspection>) (emit-page self)
				  ...
				  (let ((format (my 'format)))
					 ...
					 (let ((proc (lambda (ila)
										...
										(kdnl* 'log-horrible-screaming 'log-map (cnc self)
												 (cnc format) (map cnc (my 'variables)))
										(log-data ila self format (my 'variables))
										(kdnl* '(log-* introspection-trace) "   processed "
												 (cnc ila) " "  (procedure? ila))
										#f
										)))
;;						(dnl proc)
				  ...
				  (let ((filename (my 'filename))
					 ...
					 (cond
					  ((not (member format '(ps))) ;; svg png
						(error "<log-map> only supports postscript (and eventually svg and png)." format))
...
(model-method (<log-map> <log-introspection> <symbol>) (log-data self logger format targets)
				  (kdnl* 'log-horrible-screaming 'log-map (cnc self) (cnc logger) (cnc format) (cnc targets))
				  (lambda (target)	
					 (kdnl* '(log-* log-map) (name self) "[" (my 'name)
...
;; here as an example.
(model-method (<log-data> <log-introspection> <symbol> <list>) (log-data self logger format target-variables file)
				  (error "(-: Oops :-)")
				  (kdnl* '(log-* log-data) (name self) "[" (my 'name) ":"
							(class-name-of self) "]" "in log-data")
				  (let ((file (my 'file))
