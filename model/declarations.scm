...
(declare-method assess-distribution* "assessment routine w.r.t. spatial distribution in whole domain")
(declare-method assess-configuration "assess the configuration as a whole")
...
(declare-method data-index "map  names to column numbers, or locations to rows in an <array> agent")
(declare-method data-field-index "map  names to column numbers, or locations to rows in an <array> agent")
(declare-method data-record-index "map  names to column numbers, or locations to rows in an <array> agent")
...
(declare-method data-name "return the name of a column in an <array> agent")
(declare-method data-names "return the names of columns in an <array> agent")
(declare-method data-ref* "like data-ref, but takes lists of indices or symbols")
...
(declare-method do-assessment "assess the state of components of the  model's configuration")
...
(declare-method filter-array-fields "returns a subset of an array's fields")
(declare-method get-location "takes the agent requesting and the object of its interest")
...
(declare-method has-data? "tests to see if an <array> agent has a data field, any kind of agent has 'data' or a given slot -- extends 'has-slot?'")
(declare-method has-many-loci? "tests to see if an agent has more than one location associated with it (like arrays, or business franchises)")
...
(declare-method leaf-area* "leaf-area returns a list of the leaf area less any damage")		
(declare-method leaf-mass "leaf-mass returns the mass of the leaves on the plant")		
(declare-method leaf-mass* "leaf-mass returns a list of the mass of the leaves on the plant")		
...
(declare-method location-needs-location@ "predicate that indicates if location resolution is handled by location@ -- means we can just use (location self ...) for everything")
...
(declare-method lookup-locus "searches for a locus in a data structure returns an index") ;; locus being a generic locator (field loc [radius])
(declare-method get-loci% "returns a list of locations in a list of loci in a data structure (unsorted return)") ;; locus being a generic locator (field loc [radius])
(declare-method get-loci$ "returns a list of locations in a list of loci in a data structure (sorted return)") ;; locus being a generic locator (field loc [radius])
...
(declare-method manage-configuration "like emit-page, but for monitors")
...
(declare-method number-represented "number of entities represented by an agent")
...
(declare-method plot* "plot an agent in a <log-map> file")
...
(declare-method rec-ref "returns an entry from a list in an <array>")
(declare-method rec-set! "sets an entry from a list in an <array>")
...
(declare-method set-boundaries! "sets the centroid, radius and convex hull for an array agent")
...
(declare-method super "extracts the super field for a proxy")
(declare-method supports-proxy? "Indicates whether a thing can be proxied")
