...
(define (add-to-*default-projections* p k)
  (if (not (assoc k *default-projections*))
		...
		(error "Projection to postscript already exists" k)
		)) 
...
(model-method (<projection> <procedure>) (add-projection! self p k)
				  (set-my! 'projection-assoc-list
							  (cons (cons k p)
									  (filter (lambda (x) (not (Equal? (car x) k)))
...
(model-method (<projection> <list>) (local->model locus)
				  (let ((p (my 'local-projection)))
					 (if p (p locus)
							  locus)))

(model-method (<projection> <list>) (model->local locus)
				  (let ((p (my 'inv-local-projection)))
					 (if p (p locus)
							  locus)))
...
(model-method <projection> (get-local-projection self)
				  ;;(error "Convert to use the projections alist")
 				  ;;(my 'local-projection)
				  (my 'local-projection)
				  )
;-- get the projection that goes from local space to model space
(model-method <projection> (get-inv-local-projection self)
