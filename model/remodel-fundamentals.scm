...
The file begins with the definition of functions which are 
...
(include "remodel-framework") ;; this must be included first to load remodel-framework


;-- (lookit-q a) Examine the agents in the runqueue

(define (lookit-q a) (map (lambda (x) (x a)) (list taxon cnc subjective-time agent-state queue-state)))



;-  Variables/constants and place-holder functions both public and static
;   Would usually be overridden in model.config
...
;-- (APPLY f lst)  handles much larger lists than (apply f lst)
(define (APPLY f lst)
  (let ((N 8192))
	 (if (< (length lst) N)
		  (apply f lst)
...
;-- (PP ob)  Can pp methods as well as procedures
...
;-- (make-list k #!rest v) ... somewhat more informative.
...
;-- (slow-sort lst) Sorts objects lexically

(define (slow-sort lst) ;; 
  (let ((<=? (lambda (x y) (string<=? (object->string x) (object->string y)))))
	 (sort lst <=?)))

;-- (count-keyed-members lst) -- used by monitors, returns lists with common keys & counts
...
;-- These are unused, as far as I recall
...
;- Functions associated with the initialisation of class instances
...
;- Mathematical routines & geometric constructions

;-- (remainder* f g) used in the printing domains
...
;- Generic functions which are of use to many classes
...
;-- mass-at-age/age-at-mass functions
...
;- Functions for directly interacting with the model's subsystems (like the runqueue or other relatively "opaque" items)
...
;- Initialisation register and functions
