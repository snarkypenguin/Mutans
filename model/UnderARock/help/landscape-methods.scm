...
(define (make-circle centre radius . n)
  (make <circle> 'locus centre 'radius radius 'perimeter (Circle centre radius (if (or (null? n) (not (and (integer? (car n)) (> (car n) 2)))) 12  (car n))))
  )
  ...
  (if (memq clss (list <patch> <dynamic-patch>))
		(append (list clss)
				  (cond
					...
					((eq? bdry <polygon>)
					 (dnl* "Making patch polygon " name type centre radius box)
