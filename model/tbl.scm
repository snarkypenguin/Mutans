								  ...
								  (cond
									...
									((eq? (car x) 'external-location-mapping) ;;
									 (if (procedure? (cadr x))
											 ...
											 (error "arguments to 'external-location-mapping should be a function that maps to external location" x)))
									((eq? (car x) 'internal-location-mapping) ;;
									 (if (procedure? (cadr x))
											 (set! location->internal (cadr x))
											 (error "arguments to 'internal-location-mapping should be a function that maps to internal location" x)))
									((null? (cdr x))
									 (cond
									  ((eq? (car x) 'close)
