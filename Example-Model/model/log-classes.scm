...
(define <introspection> (make-class (inherits-from <agent>)
												(state-variables file filename filetype format variables variables-may-be-set 
...
(define <logfile> (make-class (inherits-from <introspection>) (no-state-variables)))
;; file is the output handle
;; if filename is not a string, things go to stdout
;; if append-time is not false it must either be true or the size of the number (digit count)
;; if filetype is defined it must be a string and it will be appended to the filename 
...
(define <log-data> (make-class (inherits-from <logfile>) (no-state-variables)))
...
(define <log-agent-table> (make-class (inherits-from <log-data>) (state-variables target-agent))) 
;; projections is an association list of slot-names and the projection functions to apply before output
;; Data is generated using 

(register-class <log-agent-table>)

(define <log-table> (make-class (inherits-from <log-data>) (no-state-variables))) 
;; projections is an association list of slot-names and the projection functions to apply before output
;; Data is generated using 

(register-class <log-table>)

(define <log-agent-table*> (make-class (inherits-from <log-agent-table>) (no-state-variables))) 
...
(define <log-table*> (make-class (inherits-from <log-table>) (no-state-variables))) 
