...
(define <agent> (make-class (inherits-from <object>) 
									 (state-variables name type representation agent-state
															subjective-time priority jiggle dt 
...
;;
;; priority is an integer, the higher the integer the greater the priority.  The default value is zero
;; jiggle is a real number in (0, 1).  Setting jiggle to values outside that domain suppresses the
;; the randomisation of the jiggle.
...
(define <tracked-agent> (make-class (inherits-from <agent>) (state-variables track tracked-paths track-schedule track-epsilon)))
...
(define <thing> (make-class (inherits-from <tracked-agent>) (state-variables mass dim location direction speed)))
(register-class <thing>)
;;(class-register 'add <thing> "<thing>")

(define <environment> (make-class (inherits-from <agent>) (state-variables default-value minv maxv))) ;; bounding volume
