...
;;; 			(or 
;;; 			 (symbol<? (car a) (car b))
;;; 			 (and (equal? (car a) (car b))	(> (cadr a) (cadr b))))) ;; higher orders first
;;; 	 '<)
;;; 	((and (pair? a) (pair? b)
;;; 			(symbol? (car a)) (symbol? (car b))
;;; 			(or 
;;; 			 (symbol>? (car a) (car b))
;;; 			 (and (equal? (car a) (car b))	(< (cadr a) (cadr b)))))
;;; 	 '>)
...
(define emptyset '{})
(define zerotree (node 0 emptyset))  ;; [Definition 1]
(define onetree (node 1 emptyset))  ;; [Definition 1]
	 ...
	 ((2) (let ((B (car args)) (C (cadr args)))
			  (cond
				...
				((and (polynomial? B) (node? C))
				 ...
				 (list
				  (p* B (label C))
				  (tree*ii B (children C))))
				;;(map (lambda (x) (tree*ii B x)) (children C))))

				((and (polynomial? B) (children? C))
				 ;; (dnl "polynomial and children")
				 (sort (map (lambda (x) (tree*ii B x)) C) node<? ))

				((and (number? B) (children? C))
				 ;; (dnl "polynomial and children")
				 (sort (map (lambda (x) (tree*ii (list B) x)) C) node<?)) ;; because we use multiplication by -1 to do subtraction
														 ...
														 (else 
														  (case (if (symbol? multiplier) multiplier (car multiplier))
															 ...
															 ((pairwise-multiplication-with-consolidation)
															  (n-Lambda^sigma (map* tree*ii CB CC))
															  )
															 (else (error "bad multiplication selector" multiplier))
															 )))
														node<?)))
				...
				(#f (error "bad argument to tree*ii"))
				)))
	  (else (error "too many arguments to tree*ii" args))
	  )
	 )
