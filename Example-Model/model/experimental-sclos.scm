...
;; The class <object> in the canonical sclos.scm has been changed to 
;; <primitive-object> so that <object> can be used as a class in the
;; modelling framework.  There also may be an issue in line 1009 where
;; "args" is undefined in some code .... I have no idea how that might
;; happen, and I wake during the night in a cold sweat, flinching at
;; sounds as a result. I haven't tested to see if it is Gambit specific.




;;(require 'sort)
;;(load "sort.scm") ;; either loaded or linked by the framework
...
(define <primitive-object>       (make <class>
...
;   (define <class>
;     (make <class>
;           'direct-supers (list <primitive-object>)
;           'direct-slots  (list 'direct-supers ...)))
;
(slot-set! <class> 'direct-supers      (list <primitive-object>))
									...
									(lambda args
									  (if (and (memq generic generic-invocation-generics)     ;* G  c
											...
											(apply (method-procedure                            ;* o  s
													  (lastcar (generic-methods generic)))           ;* u  e
													 (cons #f args))                              ;* n
;* d
...
(add-method initialize
				(make-method (list <primitive-object>)
								 (lambda (call-next-method object initargs)
									(display "(initialize <primitive-object>...) ")
									(newline)
									object)))
