(display "Loading landscape-methods.scm\n")
...
;; This is analogous to the "(service-list-index self sym)" call in <dynamic-patch>, but it relies on globals
			...
			(pd (lambda (species)
					(let* ((species-list service-eqn-sym-list)
							 ...
							 (cap (list-ref (list-ref service-data-list (service-index species)) 3))
							 (gr (list-ref (list-ref service-data-list (service-index species)) 4))
					  ...
					  (let ((dP/dt
								(lambda (t . populations)
								  (pop-growth-func t species cap gr as-predator-ratio predator-efficiency mort as-prey-ratio populations)))
							  )
							  dP/dt))))
...
(display "Starting dynamic-patch\n")
(add-method initialize
									...
									(if (or (pair? population-definitions)
										 ...
										 (begin
											(if (and (pair? population-definitions)
														(or (pair? d/dt-list) (pair? population-names) (pair? population-symbols)))
												 (abort (string-append
															"Dynamic patch specified a population-definition *and* one or more of\n"
															"population-names population-symbols d/dt-list\n")))
									
											(if (and (null? population-definitions)
														(not (and (pair? d/dt-list) (pair? population-names) (pair? population-symbols))))
												 (abort (string-append
															"Dynamic patch specified at least one of population-names population-symbols d/dt-list\n"
															"but not all three."
															)))

											))
									(cond
									 ((pair? population-definitions) (define-population-dynamics! self population-definitions))
									 ((pair? population-names) (define-population-dynamics! self population-names population-symbols d/dt-list))
									 (#t (slot-set! self 'population-definitions #f)
										  (slot-set! self 'population-names #f)
										  (slot-set! self 'population-symbols #f)
										  o(slot-set! self 'd/dt-list #f)))
...
(sclos-method (<dynamic-patch> <list> <list> <list>) (define-population-dynamics! self pn ps pf)
				  (if (not (and (= (length pn) (length ps) (length pf))))
						(abort "define-population-dynamics! was passed lists of differing lenghth!"))
				  (let ((tpn (apply andf (map string? pn)))
						  (tps (apply andf (map symbol? ps)))
						  (tpf (apply andf (map procedure? pf))))
					 (if (not (and  tpn tps tpf))
						  (begin
							 (display "There was at least one erroneous argument passed to define-population-dynamics!\n")
							 (if (not tpn)
...
(sclos-method (<dynamic-patch> <list>) (define-population-dynamics! self . defns )
				  (if (and (pair? defns) (pair? (car defns)) (pair? (caar defns)) (null? (cdr defns))) ;; passed a list
						(set! defns (car defns)))

				  
				  (if (not (and (apply andf (map (lambda (x) (= (length x) 3)) defns))
									 (length defns
												...
												(abort (string-append
														  ...
														  "     ("Fox" fox dfox/dt)\n"
														  "     ("Bear" bear dbear/dt))"
														  "where the arguments to each of the d/dt are t grass rabbit fox bear"
														  )
														 )
												)))
						(begin
