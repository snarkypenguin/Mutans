...
;; Basic pattern match on the structure, but a null in the instance
;; will match a symbol in the pattern
(define (wnmatch pattern instance)
  (cond
...
;; Hard-wmatch is like wmatch, but the associated values of the
;; pattern symbols have to match all the way.
...
;; Parse constructs lists of symbols (from the instance) which are
;; associated with the symbol in the pattern
...
(define (wparse pattern instance)
  ...
  (define (add-to-the-list pattern instance)
    (if (<= (length the-list) 0)
		  ...
		  (set-cdr! (list-tail the-list (1- (length the-list)))
						(cons (cons pattern instance) '()))) #t)
    ...
    (cond
     ...
     ((list? inner-pattern)
      (if (not (list? inner-instance))
			 #f
			 (if (and (> (length inner-pattern) 1)
						 (equal? (cadr inner-pattern) '...))
				  (or (and 
...
(define (wnparse pattern instance)
  ...
  (define (add-to-the-list pattern instance)
    (if (<= (length the-list) 0)
		  (set! the-list (cons (cons pattern instance) '()))
		  (set-cdr! (list-tail the-list
									  (1- (length the-list)))
						(cons (cons pattern instance) '())))
    #t)
