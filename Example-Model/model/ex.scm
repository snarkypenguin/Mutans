...
(define Pd 0.4) ; plant mort 0.8
(define Pf 0.07) ;; ~ kg fruit per kg plant
(define Px 0.0)  ; competition term ... 0.02 damps things quite a lot

(define PFx 0.0) ; Fruit on the ground inhibits plant growth -- possibly due to unmodelled processes

(define Fs 0.1) ;; spoilage  0.1
(define Sg 0.001) ; seed germination
(define Sf 1) ;; seeds in each fruit
(define Sd 0.999) ; seed mortality
(define Sx 0.3) ;; density attracting predators

(define Hp 0.08) ; herb pred 0.3
(define Hg 0.001) ; herb growth
(define Hr 0.034) ; Herb reproduction .8
(define Hd 0.02) ; Herb death
(define H_ 0.55) ;; accelerated starvation .. 3 die for each over
...
(define dP ;; This is basically dP = Pg*P - Pd + Sg*S
  (lambda (t P F S H J C c)
	 (let ((env-limit (capclip Kp P));; capacity limiting factor
			 ...
			 (growth (non-negative (- (* P Pg)  (* PFx F P)))) ;; growth of the plants, inhibited by fruit on the ground
			 (mortality Pd )
