...
contact: randall@grayrabble.org, randall@limnal.net
		...
		(if (null? i)
			 ...
			 (let loop-j ((j (seq (M 'nc))))
				(if (null? j)
					 (loop-i (cdr i))
					 (begin 
						(list-set! (list-ref m (car j)) (car i) (L M (+ (car i) 1) (+ (car j) 1)))
						(loop-j (cdr j)))))))))
...
(define (sum m M lmbda)
  (apply + (map lmbda (map (lambda (p) (+ m p)) (seq (+ (- M m) 1))))))
						...
						(cond
						 ...
						 ((and (pair? x) (= 2 N) 
						  ...
						  (if (and car-is-n cadr-is-n)
								...
								(cond 
								 (car-is-n
								  (make-matrix (list-ref-cols (list (list-ref M (- (car x) 1))) (map --1 (cadr x)))))
								 (cadr-is-n
								  (make-matrix (list-ref-rows (list-nth* M (- (cadr x) 1)) (map --1 (car x)))))
								 (else
