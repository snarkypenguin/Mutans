...
\section{Notes this construction relative to that of \Cthree}
In the development of the reference model based on the work
in \Cthree, the order of the ordinates which comprise a node proved to
be a frequent source of overhead and coding errors -- almost all
operations are on the value of a node, yet the first ordinate was the
label.  While it is true that ``wrapper'' functions could readily make
the order of the elements in the nodes irrelevant, this would come at
a cost and it seemed that fixing the problem outright would lead to
less confusion overall: in this chapter and in the reference model the
order of the elements is
\textit{value}, then \textit{label}, and finally \textit{Children}.

A number of functions from \Cthree, specifically
$\mask$, $\nmask$, $\supp$, and $\fund$, are likely to be useful in the
construction of trees associated with representing or selecting
configurations. While they have not yet been used in the reference
model or used in this chapter, they are presented here as an aid to
further application. 
...
\end{cases}
\]
where \tnode{u} and \tnode{v} are compatible.
\end{definition}
...
\begin{notation}\label{useful-set-functions}
...
\begin{notation}\label{useful-node-functions}
...
\begin{definition}\label{from-c3}
  The following functions are defined in \Cthree, and are included
  here for the sake of completeness.
 
  The support of a tree is the number of nodes which have a non-zero
  value; so,
  \begin{align*}
    \supp(\node{u}) = \begin{cases}
  ...
  which have no zero valued nodes in the nodes which like between them
  and the root node.  
  \begin{align*}
    \fund(\node{u}) = \begin{cases}
      0 & \text{ if } \nv{u} = 0 \\
  ...
  domain $[0,\Tcard{\node{u}}-1]$, since the root node must have a
  value of zero, and $\fund(\node{u}) \le \supp(\node{u})$.
...
\begin{definition}\label{Mask}
  ...
  \begin{equation}
    \excise(\node{u},\node{p}) = \begin{cases}
      ...
      \nulltree & \text{ if }  \nlabel{u} \neq \nlabel{p} \\
      (\nv{u}, \nlabel{u}, \emptyset) & \text{otherwise,}
    \end{cases}
  \end{equation}
...
$\DOMstar$, and then we will define a few useful mappings which will help
...
\begin{definition}[Notation]
  ...
  Eq. \ref{treeaddition}, by \begin{align*} \set{B}\stplus\set{C}
  = \childO{B}{C}{r}{s}{+}.  \end{align*}
\end{definition}
...
structures with a rough and ready distance function, but as the work
became more coherent, a potential underlying mathematical structure
began to emerge, and the realisation that the trees might be useful
for representing more than survey responses came about. The basic
heuristic comparisons used in exploring the survey data were replaced
with a better behaved metric based on the tree norm.

The notion of compatibility is mathematically awkward in a fashion
somewhat similar to the compatibility requirements in adding and
multiplying matrices. While the early formulation formed a non-unitary
ring, exploration suggested that restricting the operations on the
basis of compatibility may increase the trees's usefulness in a way
that might be applicable to the assessment and control of the
configuration of models comprised of an ensemble of submodels.
...
In principle, we could treat each distinct symbol in a label as a 
...
In \Cthree the model developed is concerned with demonstrating the
adaptive selection of models using abstracted representations of the
model's components and of the model's configuration. These
representations are in the form of trees in $\DOMR$ with particular
forms. In the reference model based on this example, the states of the
model as a whole, subdomains of the model and the components within
the model are represented by representative trees.  A combination of
experimentation and technical knowledge will allow us to build a set
of trees which are identified by known-good and known-bad
configurations, and the mechanism which handles switching within the
model uses the metric in its assessment.  Like the model in \Ctwo, the
approach is relatively simple, but the hope is that, having
established the ring-like properties, more advanced clustering and
