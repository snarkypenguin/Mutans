...
%% \begin{proposition}\label{itsaring} The analogue of the element
%%   \((1,1,\emptyset) \in \DOMR\) is not the multiplicative identity
%%   in the \(\DOMR\).  \begin{proof} Take an arbitrary $\rnode{u} \in
%%   \DOMR$ to be $(\rv{u}, \rlabel{u}, \rext{u})$.  Clearly the
%%   product of $\rv{u}$ and the scalar $1$ is $\rv{u}$, and the
%%   product of $\rlabel{u}$ and the polynomial label $1$ is
%%   $\rlabel{u}$.  All that remains is to observe that there is
%%   \emph{no} way we get the extension of \rnode{u} to be other than
%%   empty.
