...
with the development of the \swname{SWARM} system\citep{swarm}, and
...
Largely insisting that communication between arbitrary agents is
...
\subsection{Assessment and adaptation}
...
scoped member of the \swname{LISP} family. This means that the code
...
function/procedure definitions, \Syntax{if} statements or looping
...
identifiers (such as \symb{<animal>-model-body}) which refer to the
...
\hline & \cr
\mclass{diffeq-system} & Agents derived from this class run a
...
interactions.\cr
\hline & \cr
\mclass{model-maintenance} & Instances of this class aren't
...
\begin{table}[H]
\begin{center}
\caption{Introspection classes in \ReModel -- \fname{introspection-classes.scm}\label{classtableIIa}}
\begin{tabular}{l L{7cm}}
\toprule
\textbf{Classname} & \textbf{role} \cr
\midrule
...
\hline & \cr
\mclass{logger} & {provides the basic structure for generating model output}\cr
\hline & \cr
...
\begin{table}[H]
\begin{center}
\caption{Basic individual-based representations -- \fname{framework-classes.scm}\label{classtableIII}}
\begin{tabular}{l L{7cm}}
\toprule
\textbf{Classname} & \textbf{role} \cr
\midrule
...
\hline & \cr
\mclass{thing} & This adds mass to a \mclass{tracked-agent}.\cr
\hline & \cr
...
The more refined \mclass{example-plant} is derived from \mclass{plant}.\cr
\hline & \cr
...
by \mclass{animal-array} agents. We refrain from carrying the
generalisation to include the \mclass{carnivore} class, since
...
The the names of the files in the parameter directory (in our case
``\fname{./parameters/}) directly associate the parameter sets to
...
agent's state variables to the value \symb{<undefined>}, which is used
...
the \mclass{agent} class, namely \fname{./parameters/<agent>}.  The
...
-- we might have a generic class for oak trees -- \mclass{quercus} --
for example, which serves adequately for both the \fname{Q.robur} and
for the \fname{Q.petraea} taxa, for the most part. The parameters
...
in the \method{(create \ldots)} call used to instantiate the agent are
...
worth writing an \mclass{object} version which causes the agent
...
a \Syntax{model-body} block), next it runs any nested agents and
...
their output in a \method{log-data} method associated with their
class, but chain back to the parents \method{log-data} methods to
...
step (\variable{t} and \variable{dt}) are
always passed in by the system.  The maintenance closure can be passed
a single symbolic value -- \symb{quit} -- which indicates that it
...
purpose is \swname{$\varnothing$MQ}
(``\swname{zeroMQ}'' in \cite{0MQ2012}). No work has yet been done to
...
We can map population histograms onto piece-wise linear functions with
...
All of the output in the model arise from one of two basic mechanisms:
debugging/warning messages emitted by the \method{kdebug} routines
...
the \method{log-data} routines are able to be applied to a number of
...
alternative syntaxes \Syntax{define\%}, \Syntax{model-method\%},
and \Syntax{model-body\%} which extend 
\Syntax{define}, \Syntax{model-method},
and \Syntax{model-body} by adding instrumentation which allows the
